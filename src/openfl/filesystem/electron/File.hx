...
class File extends FileReference {
	...
	static function get_userDirectory():File {
		if (userDirectory == null)
			userDirectory = new File(Os.homedir());
		return userDirectory;
	}

	static function get_documentsDirectory():File {
		if (documentsDirectory == null)
			documentsDirectory = new File(userDirectory.nativePath + Path.sep + 'Documents');
		return documentsDirectory;
	}

	static function get_desktopDirectory():File {
		if (desktopDirectory == null)
			desktopDirectory = new File(userDirectory.nativePath + Path.sep + 'Desktop');
		return desktopDirectory;
	}

	static function get_applicationDirectory():File {
		if (applicationDirectory == null)
			applicationDirectory = new File(Path.dirname(Require.main.filename));
	...
	public function new(path:String = "") {
		...
		if (exists) {
			...
			creationDate = new Date(birthtime.getFullYear(), birthtime.getMonth(), birthtime.getDate(), birthtime.getHours(), birthtime.getMinutes(),
